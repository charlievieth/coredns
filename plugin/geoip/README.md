...
The geoip plugin uses [github.com/oschwald/geoip2-golang/v2](https://github.com/oschwald/geoip2-golang) for lookups. The v2 library reduces allocations and memory usage by switching to Go's `netip.Addr` type and a structured `Names` type instead of map-based lookups.

Internally, lookups now take `netip.Addr` rather than `net.IP`, ISO country codes use the `ISOCode` field name, and `Location.Latitude` / `Location.Longitude` are pointer fields so missing coordinates result in empty metadata values. These changes are internal to the plugin; Corefile syntax and exported metadata labels remain unchanged.
